...
The system implements 9 core features, heavily leaning on advanced SQL capabilities rather than PHP logic:

1. **Auto BMI Calculation (F1)**: Uses a MySQL `GENERATED ALWAYS AS (...) STORED` column. The BMI is calculated at the DB level, ensuring zero backend PHP logic is wasted on calculations.
2. **CDSS Alert Engine (F3)**: The heart of the platform. Instead of complex PHP loops, it uses a massive SQL `VIEW` (`CDSS_Active_Conflicts`) containing multiple `JOIN`s comparing active medications in `USER_MED_LOG` against the rules in `Drug_Conflicts`.
3. **Allergy JSON Cross-Reference (F4)**: MySQL JSON functions like `JSON_ARRAY_APPEND` and `JSON_EXTRACT` are used in the `Health_Records` table to handle array data without requiring a whole new table for allergies.
4. **Pharmacist Fulfillment (F7)**: Uses strict PDO Transactions (`BEGIN`, `COMMIT`, `ROLLBACK`). If a stock deduction fails, the prescription status update rolls back, preventing inventory desynchronization.
5. **Triggers (Automation)**:
   - `After_Med_Log_INSERT`: Automatically fires when a patient logs a new pill, checking the CDSS rules and immediately inserting a `CRITICAL` flag into `Medical_Flags` if a conflict exists.
