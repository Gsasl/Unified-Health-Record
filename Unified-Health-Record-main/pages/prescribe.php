...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['prescribe'])) {
    ...
    try {
        ...
        $pdo->rollBack();
        die("Prescription failed: " . $e->getMessage());
    }
}
...
$pageSubtitle = 'Select the best brand for your patient';

$dbmsConcept = 'Multi-table Transaction & Workflow';
$dbmsTables = 'Prescriptions & USER_MED_LOG';
$dbmsWhy = 'Demonstrates using an ACID transaction (BEGIN/COMMIT) to ensure a prescription is generated AND logged in the patient\'s active med list simultaneously. If one fails, both fail.';
