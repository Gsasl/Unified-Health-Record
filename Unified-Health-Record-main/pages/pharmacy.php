...
ini_set('display_errors', '0'); // errors go to log not page output
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fulfill'])) {
    ...
    try {
        ...
        $pdo->commit();
        $fulfillSuccess = "Rx #$rxId fulfilled successfully! Stock decremented for " . count($items) . " item(s).";
        header('Location: ' . basePath() . '/pages/pharmacy.php?msg=fulfilled');
        exit;
    } catch (Exception $e) {
        $pdo->rollBack();
...
$pageTitle = 'Pharmacy Fulfillment';
$pageSubtitle = 'ACID-compliant prescription processing';

$dbmsConcept = 'Multi-table Transaction & Stock Control';
$dbmsTables = 'Prescriptions & Brands (Stock)';
$dbmsWhy = 'Demonstrates an ACID transaction. When fulfilling a prescription, it checks stock, decrements it, marks the prescription as fulfilled, and logs the action. If stock is insufficient, the entire transaction rolls back cleanly.';
            ...
            <tr<?= $r['Status'] !== 'Pending' ? ' style="opacity:.5"' : '' ?>>
              ...
              <td>
                <?php if ($r['Status'] === 'Pending'): ?>
                  <form method="POST" action="<?= basePath() ?>/pages/pharmacy.php" style="display:inline">
                    <input type="hidden" name="fulfill" value="1">
                    <input type="hidden" name="rx_id" value="<?= $r['PrescriptionID'] ?>">
                    <button class="btn btn-sm btn-success" type="submit"
                      onclick="return confirm('Fulfill Rx #<?= $r['PrescriptionID'] ?> for <?= htmlspecialchars(addslashes($r['PatientName'])) ?>?\nStock will be decremented.')">
