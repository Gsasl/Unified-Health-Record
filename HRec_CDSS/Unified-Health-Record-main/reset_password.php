<?php
/**
 * reset_password.php — Demo Password Reset
 * Allows users to reset their password directly by providing their email.
 * Note: In a real app, this would send an email link with a token!
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email && $password) {
        ...
        if ($user) {
            ...
            $update = $pdo->prepare('UPDATE Users SET PasswordHash = ? WHERE Email = ?');
            $update->execute([$hash, $email]);

            auditLog($pdo, 'PASSWORD_RESET', 'Users', $user['UserID'], "Password reset for " . $email);
